    ...
    var body: some View {
        ...
        HStack {
            ...
            Rectangle()
                .frame(width: 80, height: 2)
                .foregroundColor(
                    password.count >= 8 && stringHasNumber(password) && stringHasSpecialCharacter(password) ? .orange : .gray)
            Rectangle()
                .frame(width: 80, height: 2)
                .foregroundColor(
                    password.count >= 8 && stringHasNumber(password) && stringHasMoreThanTwoSpecialCharacters(password) ? .green : .gray)
        }
    }
